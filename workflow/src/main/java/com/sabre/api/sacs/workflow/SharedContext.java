...
/**
 * Class contains a map of results indexed with a name of the call, indicates whether there was a fault
 * during the workflow execution and also defines how many times should the workflow be rerun in case of 
 * a failure; implements the Visitable interface, so it can be visited by the error handling visitor.
 */
public class SharedContext implements Visitable {
     ...
     * @param requestName key of the object.
     * @param result object to be stored in the context.
     */
	 ...
	 * @param conversationId conversation ID to be held by this context.
	 */
	...
	/**
	 * Returns a boolean flag indicating whether the flow had a fault during its run.
